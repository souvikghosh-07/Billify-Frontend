...
function ExpenseBills({ user, onEdit }) {
  ...
  const handleDelete = async (expenseBillId) => {
    const confirmDelete = window.confirm("Are you sure you want to delete this expense bill?");
    if (!confirmDelete) return;

    setDeletingId(expenseBillId);
    try {
      await axios.delete("https://billify-backtend.onrender.com/deleteexpensebill", {
        params: { expenseBillId: expenseBillId }
      });
      
      setBills(bills.filter((bill) => bill.expenseBillId !== expenseBillId));
      alert("Expense Bill Deleted Successfully!");
    } catch (err) {
      console.log(err);
      alert("Failed to delete expense bill!");
    } finally {
      setDeletingId(null);
    }
  };
    ...
    <>
      ...
      <div className="table-container">
        <div className="table-card">
          <h2 className="table-title">💰 My Expense Bills</h2>
          ...
          <div className="table-wrapper">
            <table className="modern-table">
              <thead>
                <tr><th>Date</th><th>Reason</th><th>Amount</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {currentBills.map((bill) => (
                  <tr key={bill.expenseBillId}>
                    ...
                    <td>
                      <div className="action-btns">
                        ...
                        <button className="delete-btn" onClick={() => handleDelete(bill.expenseBillId)} disabled={deletingId !== null}>
                          {deletingId === bill.expenseBillId ? "..." : "Delete"}
                        </button>
                      </div>
                    </td>
                  </tr>
